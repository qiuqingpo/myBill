...
import com.qiuqp.mybill.bean.Update;
...
public class UpdateManager {
	 ...
	 */
	public void checkAppUpdate(Context context,AppContext appContext,final boolean isShowMsg){
		this.mContext = context;
		this.appContext=appContext;
		if(!appContext.isNetworkConnected()&&isShowMsg)
		{
			UIHelper.ToastMessage(mContext, R.string.network_not_connected);
			return;
		}
		final int curVersionCode=getCurrentVersion();
		if(isShowMsg){
			if(mProDialog == null)
	...
	private void showNoticeDialog(){
		...
		builder.setPositiveButton("立即更新", new OnClickListener() {			
			...
			public void onClick(DialogInterface dialog, int which) {
				dialog.dismiss();
				showDownloadDialog();	
				//appContext.setUpdateLimitTime(StringUtils.getFormatTime(appContext.getMaxUpdateDate(7)));
			}
		});
		builder.setNegativeButton("以后再说", new OnClickListener() {			
			@Override
			public void onClick(DialogInterface dialog, int which) {
				appContext.setUpdateLimitTime(TimeUtils.getFormatTime(appContext.getMaxUpdateDate(2)));
				dialog.dismiss();	
			}
		});
	...
	private Runnable mdownApkRunnable = new Runnable() {	
		...
		public void run() {
			try {
				String apkName = "MyBillApp_"+mUpdate.getVersionName()+".apk";
				String tmpApk = "MyBillApp_"+mUpdate.getVersionName()+".tmp";
				//判断是否挂载了SD卡
				String storageState = Environment.getExternalStorageState();		
				if(storageState.equals(Environment.MEDIA_MOUNTED)){
					savePath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/com.mybill/Update/";
    ...
    */
	private void installApk(){
        ...
        //i.setDataAndType(Uri.fromFile(new File("file://" + apkfile.toString())), "application/vnd.android.package-archive"); 
